function ServiceCard({ icon: Icon, title, points, description, footer }) {
  return (
    <article className="group rounded-2xl border border-slate-200 bg-white p-6 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-card">
      ...
      <h4 className="mt-2 text-slate-700">{description}</h4>
      ...
      {footer && (
        <div className="mt-6 border-t border-slate-100 pt-4">
          <p className="text-sm text-slate-600 leading-relaxed italic group-hover:text-brand-700 transition">
            {footer}
          </p>
        </div>
      )}
